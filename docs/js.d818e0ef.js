...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"src/js/helpers/fetchData.js":[function(require,module,exports) {
"use strict";
...
async function dataResponse(fetch, fn) {
  ...
  let response = await fetch; // if callback function is given use it, call it, else just return the response
...
},{}],"src/js/helpers/query.js":[function(require,module,exports) {
"use strict";
...
},{"topojson-client":"node_modules/topojson/node_modules/topojson-client/index.js","topojson-server":"node_modules/topojson/node_modules/topojson-server/index.js","topojson-simplify":"node_modules/topojson/node_modules/topojson-simplify/index.js"}],"src/js/index.js":[function(require,module,exports) {
"use strict";

var _fetchData = require("./helpers/fetchData.js");

var _query = require("./helpers/query.js");
...
const svg = (0, _d.select)('svg').call(weaponTip);
const projection = (0, _d.geoMercator)();
const pathGenerator = (0, _d.geoPath)().projection(projection);
...
(0, _fetchData.getData)(url, _query.query).then(data => {
  // console.log(data.results.bindings)
  initD3(data.results.bindings); // console.log(data)
...
function initD3(places) {
  ...
  console.log(d3);
  const colorscale = d3.scaleOrdinal().domain(cats).range(d3.schemeCategory10);
  g.append('path').attr('class', 'sphere').attr('d', pathGenerator({
    type: 'Sphere'
  }));
  svg.call((0, _d.zoom)().scaleExtent([0.5, 5]).on('zoom', () => {
    var transform = _d.event.transform; // console.log(transform)
    // console.log(event.target)
    // console.log(Math.round(event.transform.k))

    g.attr('transform', _d.event.transform); // g.selectAll('circle')
    //   .attr('transform', event.transform)
  }));
  const legendWrapper = svg.append('g').attr("class", "legend-group");
  legendWrapper.append("rect").attr("x", 5).attr("class", "legend-wrapper");
  const legend = legendWrapper.selectAll(".legend").data(colorscale.domain()).enter().append("g").attr("class", "legend").attr("transform", function (d, i) {
    return "translate(0," + i * 20 + ")";
  });
  legend.append("rect").attr("x", 185 - 18).attr("width", 18).attr("height", 18).style("fill", colorscale);
  legend.append("text").attr("x", 185 - 24).attr("y", 9).attr("dy", ".35em").attr("class", "legend-text").text(function (d) {
    return d;
  });
  legendWrapper.style("transform", "translate(-200px, 203px)");
  legendWrapper.transition().duration(2000).style("transform", "translate(10px, 203px)");
  Promise.all([(0, _d.tsv)('https://unpkg.com/world-atlas@1.1.4/world/50m.tsv'), (0, _d.json)('https://unpkg.com/world-atlas@1.1.4/world/50m.json')]).then(([tsvData, topoJSONdata]) => {
    const countryName = tsvData.reduce((accumulator, row) => {
      accumulator[row.iso_n3] = row.name;
      return accumulator;
    }, {});
    const country = (0, _topojson.feature)(topoJSONdata, topoJSONdata.objects.countries);
    g.selectAll('path').data(country.features).enter().append('path').attr('class', 'country').attr('d', d => pathGenerator(d)); // .append('title')
    // 	.text(d => countryName[d.id]);

    g.selectAll('circle').data(places).enter().append('g').attr('id', d => d.title.value).append("circle").attr("r", 1).attr("transform", function (d) {
      return "translate(" + projection([d.long.value, d.lat.value]) + ")";
    }).style('fill', function (d) {
      return colorscale(d.typeLabel.value);
    }).on('mouseenter', weaponTip.show).on('mouseleave', weaponTip.hide).on('click', function (d) {
      // console.log(d3.select(this.parentNode))
      d3.select(this.parentNode).append('img').attr('src', d.picturePath.value).attr('class', 'object-image').attr("transform", function (d) {
        console.log(d);
        return "translate(" + projection([d.long.value, d.lat.value]) + ")";
      });
    });
...
},{"./helpers/fetchData.js":"src/js/helpers/fetchData.js","./helpers/query.js":"src/js/helpers/query.js","d3":"node_modules/d3/index.js","topojson":"node_modules/topojson/index.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
